...
export default function Home() {
  ...
  function createImport(event: FormEvent) { event.preventDefault(); if (!importData.url.trim() && !importData.notes.trim() && !importData.photo) { setError("Agrega al menos un enlace, nota o captura."); return; } const photo = importData.photo; const item: Hallazgo = { id: crypto.randomUUID(), capturedAt: new Date().toISOString(), photo, previewUrl: photo ? URL.createObjectURL(photo) : "", status: "needs_review", operation: "", propertyType: "", phones: [], selectedPhone: "", location: "", notes: importData.notes.trim(), ocrText: "", searchId: activeSearchId, origin: importData.origin, opportunityStatus: "new", favorite: false, url: importData.url.trim(), locationKind: "approximate" }; setHallazgos((items) => [item, ...items]); void saveHallazgo(item); setImportData({ origin: "Airbnb", url: "", notes: "", photo: null }); setView("search"); setNotice("Oportunidad importada a esta búsqueda."); }
...
function MapPanel({ items, onSelect }: { items: Hallazgo[]; onSelect: (id: string) => void }) { return <aside className="map-panel" aria-label="Mapa de oportunidades"><div className="map-grid" aria-hidden="true" /> <div className="map-copy"><strong>Mapa de {items.length} oportunidades</strong><span>● Exacta &nbsp; ○ Aproximada</span></div>{items.length === 0 ? <p className="map-empty">Las ubicaciones aparecerán aquí.</p> : items.map((item, index) => <button className={`map-marker ${item.locationKind === "exact" ? "exact" : "approximate"}`} key={item.id} onClick={() => onSelect(item.id)} style={{ left: `${18 + (index * 29) % 64}%`, top: `${22 + (index * 37) % 58}%` }} title={item.location || "Ubicación por completar"} type="button">{item.favorite ? "♥" : index + 1}<span className="sr-only">{item.locationKind === "exact" ? "Ubicación exacta" : "Ubicación aproximada"}: {item.location || "por completar"}</span></button>)}</aside>; }
